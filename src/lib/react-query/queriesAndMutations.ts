...
export const useCreatePost = () => {
    ...
    return useMutation({
        ...
        onSuccess: () => {
            //invalidateQueries makes react to fetch fresh data directly from the server and not from cache.
...
export const useGetPostById = (postId: string) => {
    return useQuery({
        ...
        enabled: !!postId
        // same id will return same data after refresh i.e. for same post
    });
}

export const useUpdatePost = () => {
    const queryClient = useQueryClient();
    //useMtation for update & useQuery for fetch
    return useMutation({
...
export const useGetPosts = () => {
    return useInfiniteQuery({
        ...
        getNextPageParam: (lastPage) => {
            // Ensure lastPage and documents exist
            if (lastPage && lastPage.documents && lastPage.documents.length > 0) {
                const lastId = lastPage.documents[lastPage.documents.length - 1].$id;
                return lastId; // Use the $id of the last document as cursor
            }
